  ...
  useEffect(() => {
    async function fetchBlogs() {
      const response = await fetch(
        "https://api.github.com/repos/RaghavendraCodes/portfolio-blogs/contents/BLOGS"
      );
      ...
      const blogsWithDates = await Promise.all(
        mdFiles.map(async (file) => {
          const commitRes = await fetch(
            `https://api.github.com/repos/RaghavendraCodes/portfolio-blogs/commits?path=BLOGS/${file.name}&per_page=1`
          );
  ...
  return (
    <div className="blogs flex flex-col mt-10">
      ...
      <div className="blogs-list flex flex-col items-center">
        {blogs.map((file) => (
          <Link to={`/blogs/blog/${file.name}`} key={file.name}>
